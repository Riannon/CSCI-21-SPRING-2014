 ...
 * Sixteenth programming challenge for CSCI 21. Complete the definition of the
 * default constructor ShoppingList, the overlaoded constructor ShoppingList,
 * the destructor ~ShoppingList and the functions getMaxItems, addItems,
 * string getItem, string& getItem, removeItem, and reset to pass the unit
 * tests. All sections labeled with "CODE HERE" need to be filled in.
 *
 * Kevan Johnson
 * Date created: 4/01/14
 * Last date modified: 4/02/14
 *
 */
...
string ShoppingList::removeItem (unsigned int index)
{
    ...
    string itemToBeRemoved = items[index];
    items[index] = "";
    if (items[index] == "")
    {
