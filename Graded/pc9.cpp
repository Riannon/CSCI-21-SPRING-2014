 ...
 * First programming challenge for CSCI 21. Complete the setName, getName,
 * setTopSpeed, getTopSpeed, setFuelSource, getFuelSource,setCrewCapacity,
 * getCrewCapacity, and toString functions to pass the unit tests. All
 * sections labeled with "CODE HERE" need to be filled in.
 *
 * Kevan Johnson
 * Date created: 2/23/14
 * Last date modified: 2/26/14
 *
 */
#include <cassert>
#include <iomanip>
...
string Spaceship::toString () const {
    ...
    catString.setf(ios_base::fixed);
    catString.setf(ios_base::showpoint);
    catString << name << ", top speed: warp ";
    catString.precision(2);
    catString << topSpeed;
    catString << ", fuel source: " << fuelSource << ", crew capacity: "
        << crewCapacity;
